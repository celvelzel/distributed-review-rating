#!/usr/bin/env python
"""DeBERTa-v3-base LoRA on 1M data - Fair comparison with 3M model.

This script uses the EXACT same training configuration as deberta_base_full.py (3M),
but with 1M data to enable fair comparison of data size impact.

Purpose: Determine if 3M data is harmful vs 1M data (same script, same config).
"""
...
CKPT_DIR = os.path.join(MODEL_DIR, "checkpoints_base_1m_fair")
...
def main():
    t_start = time.time()
    print("=" * 60, flush=True)
    print("DeBERTa-v3-base LoRA on 1M data (Fair Comparison)", flush=True)
    print("=" * 60, flush=True)

    # Load 1M tokens
    print("Loading 1M tokens...", flush=True)
    ...
    # Save
    np.save(os.path.join(MODEL_DIR, "deberta_base_1m_fair_oof.npy"), oof)
    np.save(os.path.join(MODEL_DIR, "deberta_base_1m_fair_test.npy"), avg_test)

    import pandas as pd
    pd.DataFrame({"id": test_ids, "rating": avg_test}).to_csv(
        os.path.join(OUTPUT_DIR, "submission-deberta-base-1m-fair.csv"), index=False)
    ...
    for w in [85, 90, 95]:
        ...
        pd.DataFrame({"id": test_ids, "rating": blend}).to_csv(
            os.path.join(OUTPUT_DIR, f"submission-deberta-1m-fair-ve{w}-r{100-w}.csv"), index=False)
