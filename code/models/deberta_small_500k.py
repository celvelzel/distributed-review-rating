#!/usr/bin/env python
"""DeBERTa-v3-small LoRA on 500K subsample - fits within 15GB memory limit."""
...
CKPT_DIR = os.path.join(MODEL_DIR, "checkpoints_small_500k")
...
def main():
    t_start = time.time()
    print("=" * 60, flush=True)
    print("DeBERTa-v3-small LoRA on 500K subsample", flush=True)
    print("=" * 60, flush=True)

    # Load data
    td = np.load(os.path.join(MODEL_DIR, "train_tokens_500k.npz"), allow_pickle=True)
    ...
    y_train = np.load(os.path.join(FEAT_DIR, "y_train_500k.npy")).astype(np.float32)
    ...
    for fi, (tr_idx, va_idx) in enumerate(kf.split(input_ids), 1):
        ...
        for ep in range(1, N_EPOCHS + 1):
            ...
            }, os.path.join(CKPT_DIR, f"fold{fi}_epoch{ep}.pt"))
        ...
        oof[va_idx] = np.clip(predict(model, DS(input_ids, attn_mask, ttids, idx=va_idx)), 1.0, 5.0)
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"DeBERTa-v3-small (500K, {N_FOLDS}f x {N_EPOCHS}e)", flush=True)
    ...
    np.save(os.path.join(MODEL_DIR, "deberta_small_500k_oof.npy"), oof)
    np.save(os.path.join(MODEL_DIR, "deberta_small_500k_test.npy"), avg_test)

    import pandas as pd
    pd.DataFrame({"id": test_ids, "rating": avg_test}).to_csv(
        os.path.join(OUTPUT_DIR, "submission-deberta-small-500k.csv"), index=False)

    # Blend with existing models
    ...
    for w in [90, 85, 80]:
        ...
        pd.DataFrame({"id": test_ids, "rating": blend}).to_csv(
            os.path.join(OUTPUT_DIR, f"submission-dsmall{w}-r{100-w}.csv"), index=False)
