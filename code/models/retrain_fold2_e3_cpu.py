#!/usr/bin/env python
"""Retrain fold 2 epoch 3 of DeBERTa-v3-base on CPU. Does not use GPU."""
...
def main():
    t_start = time.time()
    print("=" * 60, flush=True)
    print("Retrain fold 2 epoch 3 of DeBERTa-v3-base on CPU", flush=True)
    print("=" * 60, flush=True)
    ...
    for fi, (tr_idx, va_idx) in enumerate(kf.split(input_ids), 1):
        ...
        print(f"\nFold {fi}/3 — Loading from fold2_epoch2 checkpoint...", flush=True)
        ...
        tl = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True, num_workers=2, drop_last=True)
        vl = DataLoader(val_ds, batch_size=BATCH_SIZE*4, shuffle=False, num_workers=2)

        coral_fn = CORAL()
        opt = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
        from transformers import get_cosine_schedule_with_warmup
        steps_per_epoch = len(tl) // GRAD_ACCUM
        total_steps = steps_per_epoch * (N_EPOCHS - 2)  # only epoch 3 left
        warmup = int(steps_per_epoch * WARMUP_RATIO)
        sched = get_cosine_schedule_with_warmup(opt, warmup, steps_per_epoch)
        ...
        print(f"\nFold 2 Epoch 3 — {len(tl)} batches, ETA...", flush=True)
        ...
        for si, batch in enumerate(tl, 1):
            ...
            if si % 100 == 0:
                ...
                speed = si / elapsed
                eta = (len(tl) - si) / speed
                print(f"  f2e3 step {si}/{len(tl)}: loss={loss.item():.5f} ETA={eta:.0f}s ({speed:.3f} steps/s)", flush=True)
        ...
        with torch.no_grad():
            for batch in vl:
                ...
                mask = batch[1].to(DEVICE).long()
                tt = batch[2].to(DEVICE).long()
                vp.append(to_rating(model(ids, mask, tt)))
        ...
        ckpt_out = {"fold": fi, "epoch": ep, "model_state_dict": model.state_dict(),
                     ...
                     "best_val_rmse": vrmse, "patience_counter": 0, "completed_folds": fi - 1}
        ...
        model.eval()
        preds = []
        dl = DataLoader(test_ds, batch_size=BATCH_SIZE*4, shuffle=False, num_workers=2)
        with torch.no_grad():
            for batch in dl:
                ids = batch[0].to(DEVICE).long()
                mask = batch[1].to(DEVICE).long()
                tt = batch[2].to(DEVICE).long()
                preds.append(to_rating(model(ids, mask, tt)).numpy())
        test_preds = np.clip(np.concatenate(preds), 1.0, 5.0)
        np.save(os.path.join(MODEL_DIR, "deberta_base_fold2_test.npy"), test_preds)
        print(f"  Saved fold2 test predictions", flush=True)

        print(f"\nFold 2 epoch 3 complete! Total time: {(time.time()-t_start)/3600:.1f}h", flush=True)

        # Restore latest.txt to fold3_epoch3.pt (which was the real latest)
        with open(os.path.join(CKPT_DIR, "latest.txt"), "w") as f:
