...
Implements ALL 7 fixes from task-1 diagnosis:
    ...
    4. lr=3e-5 (was 2e-5)
    5. 5 epochs, patience=3 (was 3 epochs, patience=2)
    6. BS=16 + GradAcc=16 = effective BS=256 (was BS=64)
...
RANDOM_SEED = 42
N_FOLDS = 5
N_EPOCHS = 5              # FIX #5: was 3
PATIENCE = 3              # FIX #5: was 2
...
def train_one_fold(
    ...
    FIX #3: R-Drop — two forward passes per batch, minimize consistency
    FIX #4: lr=3e-5
    FIX #5: 5 epochs, patience=3
    FIX #6: BS=16, GradAcc=16 (effective BS=256)
    """
    ...
    train_loader = DataLoader(
        ...
        pin_memory=False,  # Disabled: saves ~4GB pinned host memory
        persistent_workers=True if NUM_WORKERS > 0 else False,
        drop_last=True,  # Drop last incomplete batch for clean GradAcc
    )
    val_loader = DataLoader(
        val_ds,
        batch_size=BATCH_SIZE * 4,  # Larger batch for eval (no grads)
        shuffle=False,
        num_workers=NUM_WORKERS,
        pin_memory=False,
    )
    ...
    # Scheduler: cosine with warmup (FIX: cosine instead of linear)
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        for step, batch in enumerate(train_loader, 1):
            ...
            # FIX #3: R-Drop — two forward passes with different dropout masks
            # Both passes computed in a single autocast context to share graph
            with autocast("cuda", enabled=FP16):
                ...
                # CORAL ordinal loss (average of two passes)
            ...
            # Accumulate metrics BEFORE freeing
            loss_val = loss.item()
            coral_val = coral.item()
            rdrop_val = consistency.item()
            n_labels = len(labels)

            # Free GPU intermediates immediately
            ...
            if step % 1000 == 0:
                ...
                # Periodic garbage collection to prevent CPU memory leak
                if step % 5000 == 0:
                    gc.collect()
                    torch.cuda.empty_cache()

        # Handle remaining gradients (if len(train_loader) not divisible by GRAD_ACCUM_STEPS)
        if step % GRAD_ACCUM_STEPS != 0:
            scaler.unscale_(optimizer)
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
            scaler.step(optimizer)
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                # Convert logits to ratings for RMSE
        ...
        # ── early stopping on RMSE (FIX #5: patience=3) ──
...
def run_cv(
    ...
    Uses same fold split as MLP for stacking compatibility.
    Uses IndexedDataset to avoid copying tensors.
    """
    n = len(y)
    ...
    for fold_idx in range(N_FOLDS):
        ...
        # Save intermediate OOF in case of crash
