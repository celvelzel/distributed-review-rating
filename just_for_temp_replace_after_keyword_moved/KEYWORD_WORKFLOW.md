...
This folder now maps to the official production pipeline in `code/models/keyword_direct_pipeline.py`.
...
- `output/submission-keyword-direct.csv` - merged final CSV when base submission is given
...
./.venv/bin/python code/models/keyword_direct_pipeline.py \
  ...
  --merged-out output/sub-deb1m-ve60-sv3rlg40-keyword.csv
```
...
3. Submit merged output CSV.

This keeps your strong model signal and only replaces rows with high-confidence direct evidence.

## Compatibility Notes

Legacy scripts in this folder still work, but now call the robust pipeline under the hood.
That means old commands continue to run while using the new anti-overfit logic.
